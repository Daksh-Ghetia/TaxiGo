...
export class UserComponent implements OnInit {
      ...
      next: (response) => {
        if (response.country.length == 0) {
          return this._toastrService.warning(
            "Please add new countries to display",
  ...
  setSelectedCountry(countryName: string) {
    const selectedCountry = this.countryList.find(
      (country) => country._id === countryName
    );
    if (selectedCountry) {
  ...
  fillDataForEdit(data: any) {
    setTimeout(() => {
      ...
      (document.getElementById("editUserId") as HTMLElement).textContent =
        data._id;
    }, 100);
  ...
  editUser(editFormData: any) {
    const id = (document.getElementById("editUserId") as HTMLElement)
      .textContent;
  ...
  searchUser() {
    if (
      this.userForm.get("userSearch").value == null ||
      this.userForm.get("userSearch").value == ""
    ) {
  ...
  cardsInfo(content: any, currentCustomerId: string) {
    ...
    this.paymentElement.on("loaderror", (event: any)=> {
      this._toastrService.error(event.error.message);
  ...
  async addPaymentDetails() {
    ...
    this._userService.addPaymentDetails(this.userId).subscribe({
      next: async (response) => {
        ...
        if (error) {
          this._toastrService.error(error.message || "Error occured while adding card","Error occured");
        } else {
          this._toastrService.success("Card added successfully", "Card Added");
        }
      },
      error: (error) => {
        this._toastrService.error(error.error.msg || "Error occured while adding payment details");
      },
      complete: () => {},
    });
  }
  ...
  async getCardDetails(customerId: string) {
    this._userService.getCardDetails(customerId).subscribe({
      next: (response) => {
        if (response.cardsData && response.cardsData.length != 0) {
  ...
  async setDefaultCard() {
    this._userService
      .setDefaultCard(this.customerDetails.id, this.selectedCardId)
      .subscribe({
        next: () => {
          this._toastrService.success("Your default card payment has been updated successfully","Card Updated");
        },
        error: (error) => {
          this._toastrService.error(error.error.msg || "Error occured while updating default card","Card Updated failed");
        },
        complete: () => {},
      });
  }
  ...
  async deleteCard(cardId: string) {
    if (confirm("Are you sure you want to delete card details")) {
      this._userService.deleteCard(cardId).subscribe({
        ...
        error: (error) => {
          this._toastrService.error(error.msg || "Error occured while deleting card","Error");
        },
        complete: () => {},
      });
    }
  ...
  async loadStripe() {
    this._settingService.getSettingData().subscribe({
      next: async (response) => {
        this.Stripe = await loadStripe(response.setting.stripePublicKey);
      }
    })
